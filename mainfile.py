...
class main:
    ...
    def load_asserts(self):
        ...
        self.bul_img = pygame.image.load(
            "images\\proto#bullet.png").convert_alpha()
        self.rock_img = pygame.image.load(
            "images\\proto#astroid.png").convert_alpha()
        self.rock_img = pygame.transform.scale(self.rock_img, (64, 64))
        self.bgm = pygame.mixer.music.load(
            "audio\\Project_Space Shooter_Final.mp3")
        pygame.mixer.music.set_volume(0.7)
        self.shoot_eff = pygame.mixer.Sound("audio\\Space Shooter_Fire.mp3")
        self.rock_exp_eff = pygame.mixer.Sound(
            "audio\\Space Shooter_Explosion.mp3")
        self.rock_impact = pygame.mixer.Sound(
            "audio\\Space Shooter_Asteroid Impact.mp3")
        self.ship_heal_eff = pygame.mixer.Sound(
            "audio\\Space Shooter_Healing.mp3")
    ...
    def gameloop(self):
        ...
        while self.running:
            rock_point = random.randint(
                0, Set.SCREEN_SIZE[0]), random.randint(0, 20)
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_j:
                        bullet.bullet(
                            self.bullet, self.ship.rect.midtop, self.bul_img)
    ...
    def _UI(self):
        healthbar = pygame.rect.Rect(
            30, Set.SCREEN_SIZE[1] - 50, self.ship.Hp * 4, 20)
        pygame.draw.rect(self.screen, self.health_bar_colour, healthbar)
        self.printf(self.screen, f"health {self.ship.Hp}",
                    (34, Set.SCREEN_SIZE[1] - 55), 'black', self.f_uwl)
        self.printf(
            self.screen, f"FPS: {round(self.clock.get_fps(), 0)}", (30, 30), 'white', self.f_pkl)
        self.printf(self.screen, f"Score : {self.score}",
                    (Set.SCREEN_SIZE[0] - 120, 20), 'white', self.f_pkl)
